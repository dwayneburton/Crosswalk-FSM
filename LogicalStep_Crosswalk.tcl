...
if {[is_project_open]} {
	if {[string compare $quartus(project) "LogicalStep_Crosswalk"]} {
		puts "Project LogicalStep_Crosswalk is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists LogicalStep_Crosswalk]} {
		project_open -revision LogicalStep_Crosswalk_top LogicalStep_Crosswalk
	} else {
		project_new -revision LogicalStep_Crosswalk_top LogicalStep_Crosswalk
	}
...
#	set_global_assignment -name STRATIX_DEVICE_IO_STANDARD "2.5 V"
	...
	set_global_assignment -name SDC_FILE LogicalStep_Crosswalk_top.out.sdc

	set_global_assignment -name SOURCE_FILE LogicalStep_Crosswalk_top.vhd
